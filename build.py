...
# SEO audit P1 fix — directional price anchor for the top screen-printing FAQ.
# Fill in real numbers (e.g. "8" and "12") and re-run build.py. While either is
# None the sentence is simply omitted, so nothing wrong ever ships.
PRICE_ANCHOR_LOW = None   # e.g. "8"  -> "$8"
PRICE_ANCHOR_HIGH = None  # e.g. "12" -> "$12"
...
def all_services():
    price_anchor = ""
    if PRICE_ANCHOR_LOW and PRICE_ANCHOR_HIGH:
        price_anchor = (f" As a rough guide, a one-color order of 24 tees typically runs "
                        f"about ${PRICE_ANCHOR_LOW} to ${PRICE_ANCHOR_HIGH} per shirt, garment included.")
    service_page(
...
<p class="updated">Minimums, turnaround, and pricing details current as of {UPDATED_HUMAN}.</p>""",
        faqs=[
            ("How much does screen printing cost?", "Every job is unique and quoted as such, based on quantity, number of colors and locations, and garment choice. Quotes are usually back within 24 hours. Bulk orders get better per-piece pricing." + price_anchor),
