...
# TODO: point at the Alliteration MailMe FOC list endpoint once it is live.
# Mirrors the same placeholder constant in the FOC repo so both flip together.
MAILME_URL = "#"
...
LOCAL_BUSINESS = {
    ...
    "@id": BASE + "/#business",
    "name": "P&M Apparel",
    "description": "Woman-owned, third-generation custom apparel company in Polk City, Iowa. Screen printing, embroidery, digital printing (DTF) and fusion transfers, sublimation, promotional products, online team stores, and live event printing for the Des Moines metro and beyond since 1987.",
    "url": BASE + "/",
    "telephone": PHONE_TEL,
    "email": EMAIL,
    "foundingDate": "1987",
    ...
    "hasMap": GBP_MAP_URL,
    "knowsAbout": ["screen printing", "custom embroidery", "digital printing", "DTF transfers", "sublimation printing", "promotional products", "online team stores", "live event printing"],
    "subOrganization": {
        "@type": "Organization",
        "name": "Iowa On Demand",
...
SERVICES = [
    ...
    ("fusion.", "/services/fusion/", "Digital printing (DTF), vinyl, glitter, rhinestones, and puff. Small orders, big detail."),
  ...
  <div class="wrap fgrid">
    ...
    <div>
      ...
      <ul class="flist">
        ...
        <li><a href="/services/fusion/">Fusion &amp; digital printing</a></li>
...
def cta_band(heading="ready when you are.", sub="Tell us what you're thinking. Quotes are usually back within 24 hours.", btns=None):
    """btns: optional pre-rendered button markup, replacing the default
    quote + call pair. Used by /flyover-con/, which routes to the event list."""
    btns = btns or (f'<a class="cta-btn" href="{QUOTE_URL}">get a quote.</a>'
...
<section class="texture band">
  <div class="wrap" style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:20px">
    ...
    <div class="btn-row" style="margin:0">{btns}</div>
  </div>
</section>"""
...
<section class="dark band">
  <div class="wrap stats">
    ...
    <div class="stat"><b>3</b><span>generations of family passing down the legacy</span></div>
    <div class="stat"><b>0</b><span>rush fees on state tournament orders</span></div>
    <div class="stat"><b>29</b><span>countries we have shipped to</span></div>
...
<section class="splitrow">
  ...
  <div class="sr-text prose">
    ...
    <p>Nobody is really buying shirts. They're buying school pride, employee culture, fundraising, memories, belonging. The shirt is just how it gets carried around.</p>
    <p>So we ask questions before we quote. We'll tell you when another method fits better, when a different blank saves you money, and when the idea you walked in with isn't going to look the way you're picturing it.</p>
    <p>What you get is one partner who cares about your project as much as you do, instead of a list of vendors to keep track of. Printing, embroidery, promo, online stores, on-demand, live events: it all happens here, so you can get back to the job you actually have.</p>
    <p>We check the art, check the order, and check the goods before they leave. It shows up on time, it looks right, and people wear it. Nobody should have to babysit a shirt order.</p>
...
def price_machine(dark=False):
    ...
    embroidery, and digital printing (DTF). Plain-HTML page content stays readable without JS;
...
<div class="pbuild" id="pbuild">
  ...
  <div class="pbuild-body">
    <div class="pb-group"><b>pick your method.</b><div class="pb-opts" data-k="mth">
      <button type="button" aria-pressed="true">screen printing</button><button type="button">embroidery</button><button type="button">digital printing</button></div></div>
    ...
    <p class="pb-fine" id="pb-note-dtf" style="display:none;margin:0 0 20px">Full color is included with digital printing (DTF). No screens, no per-color setup. Go nuts.</p>
...
(function(){
  ...
  function tipList(){
    ...
    if(sel.mth===0){
      ...
      if(sel.qty===0)t.push("Digital printing's 1-piece minimum and zero per-color setup make it the small-run champion.");
      else if(sel.qty>=2)t.push("Heads up: every digital print is placed by hand, so there's no bulk discount. At this quantity, a simple design often screen-prints for less. Ask us to quote it both ways.");
      else t.push("Every digital print is placed by hand, one at a time, so per-piece price stays flat as your order grows. No bulk discount, no bulk penalty.");
    }
...
<section class="texture hero" style="padding:84px 0 72px">
  <div class="wrap">
    ...
    <p class="lead" style="max-width:62ch">No mystery, no gotchas, no "call for pricing" runaround. Here's exactly what moves the number on your quote and how to move it in your favor.</p>
  </div>
</section>
<section>
  <div class="wrap prose">
    ...
    <p><b>1. Quantity.</b> The biggest lever by far in screen printing. Setup work (burning screens, loading the press) costs the same whether we print 12 shirts or 400, so the more pieces that setup spreads across, the less each piece carries. Price breaks land at 12, 24, 48, and 144 pieces. The exception is <a href="/services/fusion/">digital printing (DTF)</a>: every piece is placed by hand, one at a time, so its per-piece price stays flat at any quantity, no bulk discount, no bulk penalty. That's exactly why big simple orders belong on the press and small detailed ones belong on digital printing.</p>
    <p><b>2. Ink colors.</b> In screen printing, every color is its own screen, its own setup, and its own station on the press. A one-color design is the oldest budget trick in the book. (Under 12 pieces, full-color <a href="/services/fusion/">digital printing</a> sidesteps color-count math entirely.)</p>
...
{cta_band("want in on the next one?", "Get on the list and we'll send dates, speakers, and registration the moment they're set.", btns=f'<a class="cta-btn" href="{MAILME_URL}">keep me in the loop.</a>'f'<a class="cta-btn" style="background:transparent;color:#fff" href="mailto:{FOC_EMAIL}?subject=Flyover%20Con%20sponsorship">sponsor the education.</a>')}"""
...
<section class="dark">
  <div class="wrap">
    ...
    <p style="color:#dcdcdc;max-width:68ch;margin-bottom:26px">Three quick questions and we'll point you to the cleanest route. (And if the quiz is wrong, a human will happily overrule it. The quiz will not be offended.)</p>
...
(function(){{
  var Q=[
    ...
    {{q:"what's it going on?",o:[["Cotton tees or blends","cotton"],["Light polyester athletic wear","poly"],["Hats, jackets, polos, or bags","structured"],["Not apparel at all","promo"]]}}
  ];
  var R={{
    ...
    fusion:{{t:"fusion.",d:"Digital printing (DTF), vinyl, glitter, and puff. Incredible detail on small runs with a soft feel.",u:"/services/fusion/"}},
    ...
    promo:{{t:"promo products.",d:"Drinkware, bags, banners, signage, and the genuinely strange stuff. If it holds a logo, we can get it.",u:"{PROMO_URL}",l:"browse promo products.",x:1}}
  }};
  var QUIPS={{small:"One piece? Grandpa always said our minimum is 1; we can't do any less.",med:"A perfect-size batch. Right in our wheelhouse.",big:"Go big. 1 piece to 10,000 and beyond, so bring it."}};
  ...
  function done(){{
    ...
    body.innerHTML="<div class='quiz-result'><h3 class='stamp'>"+r.t+"</h3><p class='cellsub'>"+r.d+"</p><p class='cellsub quip'>"+QUIPS[a[0]]+"</p><div class='btn-row' style='margin-top:6px'><a class='cta-btn inv' href='"+r.u+"'"+(r.x?" target='_blank' rel='noopener'":"")+">"+(r.l||"see how it works.")+"</a><a class='cta-btn inv' style='background:#fff;color:#1a1a1a' href='{QUOTE_URL}'>get a quote.</a></div></div>";
  }}
...
{cta_band()}"""
    ...
    desc = "Screen printing, embroidery, digital printing (DTF), fusion transfers, sublimation, live event printing, online team stores, and promotional products in Polk City, Iowa."
...
<p>Our embroidery technicians color-match thread to existing logos on request, and you're always welcome to choose thread colors yourself. Hats, thick knits, and technical fabrics each have quirks; we'll flag anything before we stitch. Every job gets a proof approval first.</p>""",
    ...
    service_page(
        "fusion", "fusion.",
        "Digital Printing (DTF) & Vinyl Prints in Iowa | P&M Apparel",
        "Fusion decoration at P&M Apparel: digital printing (DTF), vinyl, glitter, rhinestones, and puff. Perfect for small orders, names and numbers, and detailed full-color designs. One-piece minimum.",
...
<p>Fusion is our family of heat-applied decoration: digital printing (DTF, short for direct-to-film), vinyl, glitter, rhinestones, and puff. If you've seen a photo-quality print on a single shirt, sparkly team names, or raised 3D lettering, you've seen fusion work.</p>
...
<p>Fusion plays well with everything else we do. Screen print the front, fusion the personalized names on the back. Add glitter numbers to sublimated jerseys. Your account manager will spec the combination that fits your design and budget.</p>""",
        faqs=[
            ("What is digital printing (DTF)?", "DTF stands for direct-to-film. Your design is printed in full color onto a special film, then heat-pressed onto the garment. It handles fine detail and unlimited colors with a soft feel and strong durability."),
...
<p>One-piece minimum. Standard 3 to 4 week turnaround after art approval. Pairs beautifully with fusion names and numbers for team uniforms.</p>""",
        faqs=[
            ...
            ("Can you sublimate on cotton or dark shirts?", "Sublimation requires light-colored, high-polyester fabric. For cotton or dark garments, our digital printing (DTF) method delivers similar full-color detail. We'll recommend the right method for your garment."),
        ])
...
<section>
  <div class="wrap prose">
    ...
    <p>Since 1987, we've helped businesses, schools, nonprofits, and communities create apparel people are proud to wear, through screen printing, embroidery, promotional products, online stores, and on-demand fulfillment. We've grown from Central Iowa to the whole state, the country, and lately, the world.</p>
...
<section class="texture hero" style="padding:84px 0 72px">
  <div class="wrap">
    ...
    <p class="lead" style="max-width:62ch">Walk in, call, email, or start with the quote form. Whatever's easiest for you.</p>
    <div class="btn-row"><a class="cta-btn" href="{QUOTE_URL}">get a quote.</a></div>
  </div>
</section>
...
def faq_page():
    path = "/faq/"
    faqs = [
        ("What is your minimum order?", "Like Grandpa used to say, our minimum order is 1; we can't do any less. To avoid fees we recommend 12 pieces for screen printing (24 for 4-6 colors, 48 for 7-10 colors). Embroidery and digital printing (DTF) are one-piece minimums. Promo products vary; send us what you're thinking and we'll help."),
        ...
        ("Do you ship?", "Yes. Ship or pick up; your choice. We ship across the country and overseas. Local delivery in the Des Moines metro is available by request, generally for a fee."),
...
<p>Not sure which levers apply to your order? That's literally our job. <a href="/services/">Pick a method</a>, play with the <a href="/pricing/">price machine</a>, or just ask. Quotes are free and usually back within 24 hours.</p>"""),
        ("screen printing vs digital printing (dtf): how to choose.", "/blog/screen-printing-vs-dtf/",
         "The honest decision guide: when screen printing's bulk economics win, when digital printing's one-piece minimum and full-color detail win, and how to pick for your order.",
         """
...
<h3>choose digital printing when flexibility is on yours.</h3>
<p>Digital printing (DTF, or direct-to-film, part of our fusion family) prints your design in full color onto a film, then heat-presses it onto the garment. There are no screens, so there's no color-count math and no real minimum: one piece is fine. It handles photo-real detail, gradients, and unlimited colors with a soft feel and strong durability. It's our go-to for small batches, one-offs, and player names and numbers.</p>
<h3>the quick rules.</h3>
<p>Under 12 pieces? Digital printing, almost always. Big order of a bold 1-to-3-color design? Screen printing, almost always. A design with dozens of colors or photographic detail? Digital printing unless the quantity is large enough that screen printing's setup still pays off. Personalized names on team gear? Digital printing, often combined with screen printing on the same garments: the front prints on the press, the names heat-press on the back.</p>
<p>And if you're still not sure: that's literally our job. <a href="/services/screen-printing/">Read about screen printing</a>, <a href="/services/fusion/">read about fusion and digital printing</a>, or just ask. Quotes are usually back within 24 hours.</p>"""),
...
<p>For cotton or dark garments, <a href="/services/fusion/">digital printing (DTF)</a> delivers similar full-color detail with a soft feel. For bold designs at quantity on any fabric, <a href="/services/screen-printing/">screen printing</a> is the workhorse. Part of what you get with a full-service shop is that we'll steer you to the right method for your actual garment instead of forcing your design into the wrong one.</p>
...
- [Fusion / digital printing (DTF)]({BASE}/services/fusion/)
